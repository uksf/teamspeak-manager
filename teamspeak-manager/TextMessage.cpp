...
int TextMessage::getParameterAsInt(const unsigned int index) {
    unsigned char* value = this->getParameter(index);
    if (value) {
        return int(atoi(reinterpret_cast<char*>(value)));
    }
    return 0xFFFFFFFF;
}


float TextMessage::getParameterAsFloat(const unsigned int index) {
    unsigned char* value = this->getParameter(index);
    if (value) {
        return float(atof(reinterpret_cast<char*>(value)));
    }
    return 0xFFFFFFFF;
}

unsigned char* TextMessage::getParameter(const unsigned int index) {
